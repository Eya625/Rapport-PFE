...
\begin{itemize}[left=0pt]
  \item L’administrateur est authentifié dans le système.
  \item L’administrateur est dans la section Gestion Printer Parc.
...
\begin{enumerate}[left=0pt]
  ...
  \item Le système affiche la section avec liste des interventions.
...
\section{Plan détaillé du Sprint 5}
\addcontentsline{toc}{section}{Plan détaillé du Sprint 5}

\subsection*{Introduction à la méthodologie GIMSI}
\addcontentsline{toc}{subsection}{Introduction à la méthodologie GIMSI}

La méthode \textbf{GIMSI} (Gouvernance, Investigation, Modélisation, Simulation, Industrialisation) structure notre démarche décisionnelle selon cinq phases itératives et complémentaires.  
Elle garantit à la fois un \textbf{cadre solide} (traçabilité, conformité) et une \textbf{agilité opérationnelle} (adaptation rapide, feedback continu).

\subsection*{Décodage des acronymes}
\addcontentsline{toc}{subsection}{Décodage des acronymes}

\begin{description}[leftmargin=2cm, style=nextline]
  \item[\textbf{Gouvernance}]  
    Mise en place des règles de pilotage, des comités de validation et des livrables documentés (charte, plan qualité).  
    \emph{→ Assure la cohérence stratégique et la traçabilité.}

  \item[\textbf{Investigation}]  
    Collecte, exploration et profilage des données sources (extraction OCR, APIs, base MongoDB).  
    \emph{→ Permet de révéler les anomalies et de préparer un socle fiable.}

  \item[\textbf{Modélisation}]  
    Conception du modèle décisionnel (schéma en étoile, DAX/SQL, wireframes).  
    \emph{→ Facilite les calculs de KPI et la création de prototypes visuels.}

  \item[\textbf{Simulation}]  
    Tests unitaires, cas d’usage métier, recette interne sur jeux de données représentatifs.  
    \emph{→ Vérifie la robustesse des calculs et la pertinence UX/UI.}

  \item[\textbf{Industrialisation}]  
    Automatisation via CI/CD, pipelines de mise à jour, déploiement en recette et production, documentation et formation.  
    \emph{→ Assure la reproductibilité et la montée en charge continue.}
\end{description}
...
\subsection*{Indicateur 1 : Consommation mensuelle totale}
\begin{figure}[h!]
  \centering
...
\begin{description}[leftmargin=1.5cm, style=nextline]
  \item[\textbf{Gouvernance}]  
    Définir la fréquence de mise à jour et la source officielle (collection MongoDB \texttt{consumption}).  
  \item[\textbf{Investigation}]  
    Vérification de l’intégrité des données OCR extraites des factures et du mapping mois–valeur.  
  \item[\textbf{Modélisation}]  
    DAX : \texttt{SUM(Consumption[Litre])} groupé par \texttt{Time[Month]}.  
  \item[\textbf{Simulation}]  
    Tests sur les données Q1 2025 : comparaison du script \texttt{extractor.py} avec les valeurs attendues.  
  \item[\textbf{Industrialisation}]  
    Automatisation de l’import dans Power BI via pipeline Azure Data Factory et déclenchement du rafraîchissement.
\end{description}

\bigskip

\subsection*{Indicateur 2 : Coût par agence}
\begin{figure}[h!]
  \centering
...
\begin{description}[leftmargin=1.5cm, style=nextline]
  \item[\textbf{Gouvernance}]  
    Validation des règles de calcul : \texttt{MontantTTC / NombreFactures}.  
  \item[\textbf{Investigation}]  
    Extraction des montants TTC des factures « Agency » via OCR et le script \texttt{extractor.js}.  
  \item[\textbf{Modélisation}]  
    DAX : \texttt{AVERAGE(Invoice[MontantTTC])} par \texttt{Agency[Name]}.  
  \item[\textbf{Simulation}]  
    Tests unitaires (Jest) sur la fonction de calcul avec un jeu de données de six agences.  
  \item[\textbf{Industrialisation}]  
    Mise à jour déclenchée automatiquement dès l’injection d’une nouvelle facture (via CI/CD).
\end{description}

%--- Placeholders pour les autres indicateurs (à compléter si besoin)
% \subsection*{Indicateur 3 : Facturation mensuelle}
% \subsection*{Indicateur 4 : Top 5 véhicules}
% \subsection*{Indicateur 5 : Répartition Airside/Landside}
% \subsection*{Indicateur 6 : Nombre total de véhicules actifs}
% \subsection*{Indicateur 7 : Montant moyen par facture}
